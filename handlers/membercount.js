...
module.exports = function (client, options) {
  ...
  client.Jobmembercount = new CronJob('0 0 * * * *', async function() {
    ...
    console.log(JSON.stringify(client.guilds.cache.map(guild => `${guild}`)).italic.yellow + " MEMBERCOUNTER ALL GUILDS")
  ...
  client.on("ready", async () => {
    ...
    var logguilds = client.guilds.cache.size;
    console.log(` ${client.guilds.cache.size}`.yellow + " MEMBERCOUNTER ALL GUILDS")
    //Loop through all guilds and send a random auto-generated-nsfw setup
    for(const guildid of guilds){
        memberCount(guildid)
        await delay(1000);
