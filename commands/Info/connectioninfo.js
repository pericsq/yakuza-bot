  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    let ls = client.settings.get(message.guild.id, "language")
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .addField('<:arrow:905708227998670868> **Channel**', `> **${member.voice.channel.name}** ${member.voice.channel}`, true)
        .addField('<:arrow:905708227998670868> **Channel-ID**', `> \`${member.voice.channel.id}\``, true)
        .addField('<:arrow:905708227998670868> **Members in there**', `> \`${member.voice.channel.members.size} total Members\``, true)
        .addField('<:arrow:905708227998670868> **Full Channel?**', `> ${member.voice.channel.full ? "✅" : "❌"}`, true)
        .addField('<:arrow:905708227998670868> **Bitrate**', `> ${member.voice.channel.bitrate}`, true)
        .addField('<:arrow:905708227998670868> **User join limit**', `> \`${member.voice.channel.userLimit != 0 ? member.voice.channel.userLimit : "No limit!"}\``, true)
