  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      async function first_layer(){
        ...
        .setColor(es.color)
        .setAuthor('Owner Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/prohibited_1f6ab?.png',  'https://easy-code.ro')
        ...
        client.on('interactionCreate',  (menu) => {
          if (menu?.message.id === menumsg.id) {
            if (menu?.user.id === cmduser.id) {
              if(used1) return menu?.reply({content : `<a:Error:902091954425298956> You already selected something, this Selection is now disabled!`, ephermal : true});
              menuselection(menu);
            }
            else menu?.reply({content : `<a:Error:902091954425298956> You are not allowed to do that! Only: <@${cmduser.id}>`, ephermal : true});
          }
        });
      }
            ...
            await tempmsg.channel.awaitMessages({filter: m => m.author.id === message.author.id,
              ...
              .then(collected => {
                ...
                if (user) {
                  ...
                  if(user.id == "754977956316577802"){
                    return message.channel.send({embeds: [new MessageEmbed()
                      .setFooter(client.getFooter(es))
                      .setColor(es.wrongcolor)
                      .setTitle(`You can't remove Fane!`)
