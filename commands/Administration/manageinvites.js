...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      .addField("**CURRENT INVITES:**", `<a:neige:939814994328043540> ${user} _**has invited __${realinvites} Member${realinvites != 1 ? "s": ""}__**_!`)
      ...
      async function menuselection(menu) {
        ...
        await menu?.reply({embeds : [new MessageEmbed()
          .setColor(es.color)
          .setAuthor(client.la[ls].cmds.info.botfaq.menuembed.title, client.user.displayAvatarURL(), "https://easy-code.ro")
          ...
          message.reply({embeds : [new MessageEmbed()
            .setAuthor(`New Invites of: ${user.tag}`, user.displayAvatarURL({dynamic: true}), "https://easy-code.ro")
            .setColor(es.color).setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
            .addField("\u200b", `<a:neige:939814994328043540> ${user} _**has invited __${realinvites} Member${realinvites != 1 ? "s": ""}__**_!`)
