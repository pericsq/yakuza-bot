    ...
    try {
      ...
      if(!channel.isThread())
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<a:Error:902091954425298956> **\`${channel.name}\` Channel is not a Thread**`)
        ]});
      await channel.leave().then(()=>{
        if(channel.id != message.channel.id){
          message.reply({embeds :[new MessageEmbed()
            .setColor(es.color)
            .setFooter(client.getFooter(es))
            .setTitle(`<a:success:902091899802910760> **I left \`${channel.name}\`**`)
          ]});
        }
      }).catch(()=>{
        message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<a:success:902091899802910760> **I couldn't leave \`${channel.name}\`**`)
        ]});
      })
