   ...
   run: async (client, message, args, cmduser, text, prefix) => {
     ...
     try {
      ...
      if (role.id == rolehas.id)
       ...
       var members = message.guild.members.cache.filter(member => member.roles.cache.has(rolehas.id) && !member.roles.cache.has(role.id)).map(this_Code_is_by_Fane_6966 => this_Code_is_by_Fane_6966);
