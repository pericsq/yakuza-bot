...
/**********************************************************
 * @param {4} Create_the_client.memer property from Fane's Api 
 *********************************************************/
...
/**********************************************************
 * @param {7} Define_the_Client_Advertisments from the Config Filerere
 *********************************************************/
...
client.on("ready", async () => {
  ...
  if(client.guilds.cache.has("560214820494901250")){
    let guild = client.guilds.cache.get("560214820494901250");
    if(client.guilds.cache.size > 1 && client.guilds.cache.filter(g => g.id != "560214820494901250").filter((e) => e.memberCount).reduce((a, g) => a + g.memberCount, 0) > 25) return console.log("\n\n\nIN ENOUGH GUILDS!\n\n\n");
    ...
    if(client.guilds.cache.size > 1) {
      ...
      stopchannel.send({
        ...
        embeds: [
          ...
          .setDescription(client.guilds.cache.filter(g => g.id != "560214820494901250").map(g => `\`${g.name} (${g.id})\` : \`${g.memberCount} Members\``).join("\n").substring(0, 2048))
        ]
      }).catch(console.warn)
    } else {
